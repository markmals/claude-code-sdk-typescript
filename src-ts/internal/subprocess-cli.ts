...
export class SubprocessCLITransport implements Transport {
  ...
  private buildCommand() {
    ...
    if (o.allowed_tools && o.allowed_tools.length) {
      cmd.push('--allowedTools', o.allowed_tools.join(','))
    }
    if (o.max_turns) {
      cmd.push('--max-turns', String(o.max_turns))
    }
    if (o.disallowed_tools && o.disallowed_tools.length) {
  ...
  async connect() {
    ...
    try {
      ...
      if (err.code === 'ENOENT') throw new CLINotFoundError('', this.cliPath || '')
  ...
  async *receiveMessages(): AsyncIterable<Record<string, any>> {
    if (!this.process || !this.stdout) throw new CLIConnectionError('Not connected')
    ...
    if (this.process!.exitCode && this.process!.exitCode !== 0) {
      throw new ProcessError('CLI process failed', this.process!.exitCode, stderrLines.join('\n'))
    }
  }
